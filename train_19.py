...
def cal_loss(model, images, labels_1, labels_2, labels_3, labels_4, object_buf):

    with tf.GradientTape() as tape:
        patch_1, patch_2, patch_3, patch_4 = run_model(model, images, True)
        patch_1 = tf.reshape(patch_1, [-1, ])
        patch_2 = tf.reshape(patch_2, [-1, ])
        patch_3 = tf.reshape(patch_3, [-1, ])
        patch_4 = tf.reshape(patch_4, [-1, ])
        ...
        temp_count_1 = np.bincount(labels_1.numpy(), minlength=2)
        temp_count_2 = np.bincount(labels_2.numpy(), minlength=2)
        temp_count_3 = np.bincount(labels_3.numpy(), minlength=2)
        temp_count_4 = np.bincount(labels_4.numpy(), minlength=2)

        if temp_count_1[1] == 0:
            loss_1 = false_dice_loss(labels_1, patch_1)
        else:
            loss_1 = true_dice_loss(labels_1, patch_1)

        if temp_count_2[1] == 0:
            loss_2 = false_dice_loss(labels_2, patch_2)
        else:
            loss_2 = true_dice_loss(labels_2, patch_2)

        if temp_count_3[1] == 0:
            loss_3 = false_dice_loss(labels_3, patch_3)
        else:
            loss_3 = true_dice_loss(labels_3, patch_3)

        if temp_count_4[1] == 0:
            loss_4 = false_dice_loss(labels_4, patch_4)
        else:
            loss_4 = true_dice_loss(labels_4, patch_4)
...
def main():
    ...
    model = patch_flow_Unet(input_shape=(FLAGS.img_size, FLAGS.img_size, 3))
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                patch_labels_1 = batch_labels[:, 0:FLAGS.img_size // 2, 0:FLAGS.img_size // 2, :]
                patch_labels_2 = batch_labels[:, 0:FLAGS.img_size // 2, FLAGS.img_size // 2:, :]
                patch_labels_3 = batch_labels[:, FLAGS.img_size // 2:, 0:FLAGS.img_size // 2, :]
                patch_labels_4 = batch_labels[:, FLAGS.img_size // 2:, FLAGS.img_size // 2:, :]
                ...
                loss = cal_loss(model, batch_images, patch_labels_1, patch_labels_2, patch_labels_3, patch_labels_4, object_buf)

                if count % 10 == 0:
                    print("Epochs: {}, Loss = {} [{}/{}]".format(epoch, loss, step + 1, tr_idx))

                if count % 100 == 0:
                    patch_1, patch_2, patch_3, patch_4 = run_model(model, batch_images, False)
                    patch_12 = tf.concat([patch_1, patch_2], 2)
                    patch_34 = tf.concat([patch_3, patch_4], 2)
                    final = tf.concat([patch_12, patch_34], 1)
                    for i in range(FLAGS.batch_size):
            ...
            for i in range(tr_idx):
                ...
                for j in range(FLAGS.batch_size):
                    ...
                    patch_1, patch_2, patch_3, patch_4 = run_model(model, batch_image, False)
                    patch_12 = tf.concat([patch_1, patch_2], 2)
                    patch_34 = tf.concat([patch_3, patch_4], 2)
                    final = tf.concat([patch_12, patch_34], 1)
