...
def cal_loss(model, batch_images, batch_labels, object_buf):
    ...
    with tf.GradientTape() as tape:
        ...
        total_loss = bce_loss + (loss_1 + loss_2 + loss_3 + loss_4 + loss_5) / 5.
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                if count % 100 == 0:
                    final, sub_1, sub_2, sub_3, sub_4, _ = run_model(model, batch_images, False)
                    final = tf.reduce_mean(final + sub_1 + sub_2 + sub_3 + sub_4, -1)
                    for i in range(FLAGS.batch_size):
            ...
            for i in range(tr_idx):
                ...
                for j in range(FLAGS.batch_size):
                    ...
                    final, sub_1, sub_2, sub_3, sub_4, _ = run_model(model, batch_image, False)
                    final = tf.reduce_mean(final + sub_1 + sub_2 + sub_3 + sub_4, -1)
